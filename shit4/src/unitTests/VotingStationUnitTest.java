...
public class VotingStationUnitTest {
	private List<String> passwords = Arrays.asList("password1", "password2",
			"password3");
	private IVotingStation votingStation = new VotingStation(passwords,
			"voting station 1", new ChoosingListFactoryStub(),
			new ChoosingWindowFactory(), new VotingStationWindowFactoryStub(),
			new Main_Window(new Translate2EnglishFactory()));
	...
	public void wrong_password_testVoting_Test()
	...
	public void testVoting_Test1() throws ChoosingInterruptedException {
		...
		votingStation.testVoting();
		assertEquals(VotingStationWindowStub.getMessage(),
				"You successfully test voted for the party likoud");
	}
	...
	public void voting_Test3() throws ChoosingInterruptedException {
		...
		votingStation.voting();
		assertEquals(VotingStationWindowStub.getMessage(),
				"You successfully voted for the party likoud");
		Driver_Mainframe.setPlace(2);
		ChoosingListStub.setPartyNumber(1);
		votingStation.voting();
		assertEquals(VotingStationWindowStub.getMessage(),
				"You successfully voted for the party kadima");
		ChoosingListStub.setPartyNumber(2);
		votingStation.voting();
		assertEquals(VotingStationWindowStub.getMessage(),
				"You successfully voted for the party avoda");
		votingStation.voting();
	}
