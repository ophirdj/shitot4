...
import unitTests.practiceStation.ChoosingListStub.ChooseListLongComponent;
import unitTests.practiceStation.ChoosingListStub.ChooseListTooLongComponent;
...
import unitTests.practiceStation.ImagePanelStub.ShowFirstImageLongComponent;
import unitTests.practiceStation.ImagePanelStub.ShowFirstImageTooLongComponent;
...
public class PracticeTestPaths {
	...
	/**
	 * @return The time guard: time that assured the station finish basic calls.
	 */
	...
	/**
	 * Test to paths:
	 * First a test with several paths that only after the final
	 * one time run out.
	...
	/**
	 * Enum that represent the different amount of time to wait:
	 * None: not at all. Some: wait, but should still have time. 
	 * TooLong: timeout before finishing
	 */
	public enum WaitingInterval{
		None{
			@Override
			public void chooseParty(TestEnvironment testEnviroment,IParty party,long waitTime){
				testEnviroment.addComponentForTest(new ChooseListComponent(party));
			}
			
			@Override
			public void showGuide(TestEnvironment testEnviroment,Languages language, long waitTime){
		...
		public abstract void chooseParty(TestEnvironment testEnviroment,IParty party, long waitTime);
		public abstract void showGuide(TestEnvironment testEnviroment,Languages language, long waitTime);
	...
	public static class PartyParameters{
		...
		public long Choose(TestEnvironment testEnviroment,boolean isFinal,long timeLeft){
	...
	public static class GuideParameters{
		...
		public long showGuide(TestEnvironment testEnviroment, long timeLeft) {
	...
	public static class PathesParameters{
		...
		private void retirePath(TestEnvironment testEnviroment) {
		...
		public void buildPathes(TestEnvironment testEnviroment){
	...
	public static class PathParameters{
		...
		public long toShowGuide(TestEnvironment testEnviroment,long timeLeft){
			timeLeft = guide.showGuide(testEnviroment,timeLeft);
			return timeLeft;
		}
		
		public long reChooseParties(TestEnvironment testEnviroment, long timeLeft){
